<div>
    ...
    <div class="py-1 d-flex gap-2 align-items-center">
        ...
        <a href="{{ route('addAgreement', ['ownerId' => $owner->id]) }}"
        ...
        <x-slot name="body">
            @foreach ($this->loadAgreements as $index => $agreement)
                <tr wire:key="{{ $agreement->id }}"
                    class="rental-agreement-table {{ $agreement->remaining_days_color }}">
                            ...
                            <div aria-label="Action Buttons">
                                @if ($agreement->agreement_status == 'Submitted')
                                    <button type="button" class="dropdown-item rounded-top text-success"
                                        wire:click="approve({{ $agreement->id }})">
                                ...
                                @if ($agreement->agreement_status !== 'Approved')
                                    ...
                                    <x-table.action-option type="delete"
                                        ...
                                        wire:click="delete({{ $agreement->id }})" />
                                ...
                                @endif
                                <button type="button" class="dropdown-item rounded-top text-dark"
                                    wire:click="view({{ $agreement->id }})">
                                    <i class="bi bi-file-text"></i> View
                                </button>
                                <button type="button" class="dropdown-item rounded-top text-dark"
                                    wire:click="copy({{ $agreement->id }})">
                                    <i class="bi bi-files text-black"></i> Copy
                                </button>
                                <button class="dropdown-item rounded-top text-blue" type="button"
                                    wire:click="export({{ $agreement->id }})">
                    ...
                    <x-modal id="model">
                        ...
                        <x-slot name="footer">
                            ...
                            <x-form.button type="submit" wire:click="addTerminateDate({{ $agreement->id }})"
                    ...
                    <x-modal id="modals">
                        ...
                        <x-slot name="footer">
                            <x-form.button color="secondary" data-bs-dismiss="modal">Cancel</x-form.button>
                            <x-form.button type="submit" wire:click="addReason({{ $agreement->id }})"
                                color="primary">Save</x-form.button>
                        </x-slot>
                    </x-modal>
                    <x-modal id="modal">
                        ...
                        <x-slot name="footer">
                            <x-form.button color="secondary" data-bs-dismiss="modal">Cancel</x-form.button>
                            <x-form.button type="submit" accept="application/pdf,image/*"
                                wire:click="uploadDocument({{ $agreement->id }})"
                                color="primary">Save</x-form.button>
                        </x-slot>
                    </x-modal>
                </tr>
