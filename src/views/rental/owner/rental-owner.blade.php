<div>
    ...
    <x-table.table-wrapper>
        ...
        <x-slot name="header">
            ...
            <x-table.heading :sortable="true" col="CitizenshipImage" :sortBy="$sortBy" :sortDirection="$sortDirection">Citizenship
                Image</x-table.heading>
            <x-table.heading :sortable="true" col="ChequeImage" :sortBy="$sortBy" :sortDirection="$sortDirection">Cheque
                Image</x-table.heading>
            <x-table.heading :sortable="true" col="Status" :sortBy="$sortBy" :sortDirection="$sortDirection">Agreement
                Status</x-table.heading>
            <x-table.heading :sortable="true" col="Status" :sortBy="$sortBy"
                :sortDirection="$sortDirection">Status</x-table.heading>
        ...
        <x-slot name="body">
            @foreach ($this->list as $index => $owner)
                <tr wire:key="{{ $owner->id }}">
                    ...
                    <x-table.cell>
                        @if ($owner->documents->where('type', 'cheque')->isNotEmpty())
                            @foreach ($owner->documents->where('type', 'cheque') as $document)
                                <a href="{{ asset('storage/' . $document->image_path) }}" target="_blank">
                                    <img alt="Cheque Document" src="{{ asset('storage/' . $document->image_path) }}"
                                        style="max-width: 100px; height: auto;" />
                                </a>
                            @endforeach
                        @else
                    ...
                    <x-table.cell>
                        <div class="form-group col-md-4">
                            @php
                                $latestAgreement = $owner->agreementStatus->sortByDesc('agreement_date')->first();
                            @endphp

                            @if ($latestAgreement)
                                @if ($latestAgreement->agreement_status == 'Submitted')
                                    <div class="flex items-center gap-2">
                                        <button
                                            class="btn btn-primary">{{ $latestAgreement->agreement_status }}</button>
                                        <span>({{ $latestAgreement->agreement_date }} to
                                            {{ $latestAgreement->agreement_end_date }})</span>
                                    </div>
                                @elseif ($latestAgreement->agreement_status == 'Approved')
                                    <div class="flex items-center gap-2">
                                        <button
                                            class="btn btn-success">{{ $latestAgreement->agreement_status }}</button>
                                        <span>({{ $latestAgreement->agreement_date }} to
                                            {{ $latestAgreement->agreement_end_date }})</span>
                                    </div>
                                @elseif ($latestAgreement->agreement_status == 'Rejected')
                                    <div class="flex items-center gap-2">
                                        <button
                                            class="btn btn-danger">{{ $latestAgreement->agreement_status }}</button>
                                        <span>({{ $latestAgreement->agreement_date }} to
                                            {{ $latestAgreement->agreement_end_date }})</span>
                                    </div>
                                @endif
                        ...
                        <x-table.action>
                            <div role="group" aria-label="Action Buttons">
                                @if ($owner->rental_status == 'Submitted')
                                    <button type="button" class="dropdown-item rounded-top text-success"
                                        wire:click="approve({{ $owner->id }})">
                                        <i class="bi bi-check-circle"></i> Approve
                                    </button>

                                    <button type="button" class="dropdown-item rounded-top text-danger"
                                ...
                                @if ($owner->rental_status != 'Approved')
                                    <x-table.action-option type="edit" wire:click="edit({{ $owner->id }})" />
                ...
                <x-modal id="modal">
                    ...
                    <x-slot name="footer">
                        <x-form.button type="submit" wire:click="addReason({{ $owner->id }})"
                            color="primary">Save</x-form.button>
                    </x-slot>
                </x-modal>
            @endforeach
        </x-slot>
    ...
    <style>
        .bg-row-terminate {
            background-color: rgba(185, 16, 16, 0.1) !important;
            /* Light Red */
        }

        .bg-row-warning {
            background-color: rgba(185, 182, 16, 0.1) !important;
            /* Light Yellow */
        }
    </style>
</div>
